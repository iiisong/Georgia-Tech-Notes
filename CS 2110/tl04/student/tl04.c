...
int stack_push(const char *question, enum category_t category) {
    ...
    if (!question || !strlen(question)) return FAILURE;
    ...
    if (numPosts == capacity) {
        ...
        for (int i = 0; i < capacity / 2; i++) {
            // stack_arr[i + 1] = old_stack_arr[i];
            stack_arr[i] = old_stack_arr[i];
        }
        free(old_stack_arr);
...
int stack_pop(struct post_t *data) {
    ...
    if (!data || !numPosts) return FAILURE;
    
    numPosts -= 1;
    data->question = stack_arr[numPosts]->question;
    data->category = stack_arr[numPosts]->category;
    free(stack_arr[numPosts]);
