...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; i++) {
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer + col + (i + row) * WIDTH;
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  // waitForVBlank();
  for (int i = 0; i < height; i++) {
    DMA[3].src = image + col + (i + row) * WIDTH;
    DMA[3].dst = videoBuffer + col + (i + row) * WIDTH;
    DMA[3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
...
/*
  Draws a rectangular chunk of an arbitrary image to the video buffer.
  The width and height, as well as the top left corner of the chunk to be drawn, are passed as parameters.
  The width of image must be less than HEIGHT and the height of image must be less than HEIGHT.
  This function can be completed using `height` DMA calls.
*/
void drawImageSectionDMA(int row, int col, int src_row, int src_col, int width, int height, const u16 *image) {
  // TODO: IMPLEMENT
  // waitForVBlank();
  for (int i = 0; i < height; i++) {
    DMA[3].src = image + src_col + (i + src_row) * WIDTH;
    DMA[3].dst = videoBuffer + col + (i + row) * WIDTH;
    DMA[3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
  }
}
