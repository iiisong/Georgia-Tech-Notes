 ...
 * Collaborators: None
 *
 * Resources: QZ JUnits, Lucian JUnits, Kothapalli JUnits, Sahi JUnits
 */
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        int hc = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> entry = table[hc];
        ...
        table[hc] = new ExternalChainingMapEntry<K, V>(key, value, table[hc]);
        size += 1;
        return null;
    }
    ...
    public V remove(K key) {
        ...
        while (entry.getNext() != null) {
            if (key.equals(entry.getNext().getKey())) {
                V result = entry.getNext().getValue();
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length cannot be less than size");
        }
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            while (entry != null) {
                counter += 1;
                int hc = Math.abs(entry.getKey().hashCode() % newTable.length);
                newTable[hc] = new ExternalChainingMapEntry<K, V>(entry.getKey(), entry.getValue(), newTable[hc]);
                entry = entry.getNext();
            }
        }
